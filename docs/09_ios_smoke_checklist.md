...
- [ ] **2x3 grid** 6개 카드 모두 정상 진입
- [ ] **117 긴급 배너** tap → iOS 전화 앱 호출 시트 → 통화 동작
- [ ] 신고 후 grid 두 번째 카드가 "**내 사안 보기**" 로 자동 변경
- [ ] **대화 이력** — SQLite 영속화 → 앱 재시작 후에도 유지

## 3. 신고/진행/알림 (Stage 3 + 4)

- [ ] 신고하기 3-step 폼 — 모든 chip / multi-select / TextField 동작
- [ ] 제출 → R-번호 발급 화면 → **클립보드 복사** (iOS 클립보드 작동)
- [ ] 인박스에 milestone 자동 적재
- [ ] 학생/보호자: **사안 상세** 화면에 read-only + "AI에 다음 단계 물어보기" 버튼
- [ ] 교사: 사안 상세 → "**다음 단계로 진행**" → 인박스 milestone 추가 + KPI 갱신
- [ ] 교사: 우상단 ⋯ → **사안 삭제** → 확인 → 목록에서 사라짐
