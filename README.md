...
3. **홈** (`/`) — 단일 홈. 챗봇·사안 정리·법령·FAQ·절차·대화 이력·117 모두 한 화면에서 진입.

> 동의 전까지 라우터 redirect 로 다른 경로 진입을 막는다. 역할 선택 단계 없음 (2026-04-27 옵션 A 적용).

### 포지셔닝 — "개인 사안 노트" (모델 D)

- 본 앱의 "사안 정리" 는 **본인 기기에 저장하는 개인용 메모**입니다. 학교·교육청·경찰에 자동 전달되지 않습니다.
- "단계 변경" 은 외부 채널(학교/117) 과 본인이 진행한 결과를 노트에 기록하는 의미의 체크리스트입니다.
...
[면책] → [인트로] → [홈]
       ...
       ┌─────────────┼──────────────────────┬──────────────┐
       ▼             ▼                      ▼              ▼
    [챗봇]    [사안 정리 3-step]       [내 사안 노트]    [법령/FAQ/절차]
       │             │                      │              │
       │             ▼                      ▼              │
       │      [메모 번호 발급]         [노트 상세]            │
       │      (본인 기기 저장)              │                │
