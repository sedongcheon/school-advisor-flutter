...
└── features/
    ...
    ├── chat/                  # 실연동 SSE + 마크다운 + 인용 칩 + 답변 신고
    ├── laws/                  # GET /laws + 조문 바텀시트
    ├── user/                  # GET /user/status + 사용량 칩
    ├── feedback/              # POST /feedback (답변 평가/신고)
    ├── report/                # 사안 정리 노트 (로컬 SQLite, 자동 전달 X)
    └── debug/                 # /health 디버그 화면
shared/
├── utils/
...
4. **홈** (`/`) — 역할별 진입 화면 (학생: 챗봇·사안 정리·내 사안 노트 / 보호자: 정리한 사안 카드 / 교사: 사안 노트 리스트).

> 동의·역할 선택 전까지 라우터 redirect 로 다른 경로 진입을 막는다.

### 포지셔닝 — "개인 사안 노트" (모델 D)

- 본 앱의 "사안 정리" 는 **본인 기기에 저장하는 개인용 메모**입니다. 학교·교육청·경찰에 자동 전달되지 않습니다.
- 보호자/교사 모드는 데이터가 다른 별개 영역이 아니라 **같은 노트의 다른 톤·메뉴**입니다 (어떤 역할을 골라도 같은 기능 사용 가능).
- 실제 신고는 학교 학교폭력 전담기구 또는 117 (학교폭력 신고센터) 로 직접 연락. 앱은 그 자료 정리·법령 안내·챗봇 가이드를 담당합니다.

### 전체 사용자 흐름

```
[면책] → [인트로] → [역할 선택 (관점·톤 선택, 데이터 격리 X)]
                      │
       ┌──────────────┼─────────────────┐
       ▼              ▼                 ▼
   [학생 홈]      [보호자 홈]        [교사 홈]
       ...
       │  ┌──────────┴───────┐          │
       ▼  ▼                  ▼          ▼
   [챗봇]  [사안 정리 3-step]  [내 사안 노트]   [사안 노트 리스트]
       │       │               │            │
       │       ▼               ▼            ▼
       │   [메모 번호 발급]  [노트 상세]    [노트 상세]
       │   (본인 기기 저장)        │
       ▼                       ▼
   [법령 검색]           [단계 진행 / 삭제]
   [FAQ / 절차도]              ─ 단계는 본인이 수동으로 옮김 ─
   [알림 인박스] ◀──── (단계 변경 / 결제 / 푸시)
   [설정 → 결제 / 알림 / 테마]

   ※ 실제 신고는 [117] 또는 [학교 전담기구] 로 외부 연락 (앱 외부)
```
